...
import SlopeMeter, { SlopeReading } from '@/components/ui/SlopeMeter'
...
// 'slope' sits between 'ball' (confirmed) and 'walk'
type Phase = 'ball' | 'slope' | 'walk' | 'athole' | 'analyzing' | 'done'
...
export default function CameraViewfinder({ onAnalysisComplete, onReset, confirmedGreenSpeed }: CameraViewfinderProps) {
  ...
  const [slope, setSlope]           = useState<SlopeReading | null>(null)
  ...
  useEffect(() => {
    ...
    return () => { if (watchIdRef.current !== null && navigator.geolocation) { navigator.geolocation.clearWatch(watchIdRef.current); watchIdRef.current = null } }
  }, [phase])
  ...
  const draw = useCallback(() => {
    ...
    if (ps === 'done' && an) {
      const bx=W*0.5, by=H*0.12, hx=W*0.5, hy=H*0.88
      const ddx=hx-bx, ddy=hy-by, len=Math.sqrt(ddx*ddx+ddy*ddy)||1
      ...
      const cpx=(bx+hx)/2+perpX*offset, cpy=(by+hy)/2+perpY*offset

      // Slope arrows
      if (an.slope !== 'flat') {
        const ss=an.slope==='downhill'?1:-1, lf=an.breakDirection==='left'?0.4:an.breakDirection==='right'?-0.4:0
        const rX=(ddx/len)*ss+perpX*lf, rY=(ddy/len)*ss+perpY*lf
        const rL=Math.sqrt(rX*rX+rY*rY)||1, aX=(rX/rL)*7, aY=(rY/rL)*7, ha=Math.atan2(aY,aX)
        ctx.strokeStyle='rgba(76,175,80,0.3)'; ctx.lineWidth=1; ctx.setLineDash([])
        for (let gi=1;gi<=4;gi++) for (let gj=1;gj<=4;gj++) {
          const ix=(W/5)*gi, iy=(H/5)*gj, tx=ix+aX, ty=iy+aY
      ...
      grad.addColorStop(0,'rgba(76,175,80,0.95)'); grad.addColorStop(0.6,'rgba(180,230,180,0.95)'); grad.addColorStop(1,'rgba(255,255,255,0.95)')
      ctx.beginPath(); ctx.moveTo(hx,hy); ctx.quadraticCurveTo(cpx,cpy,bx,by)
      ctx.strokeStyle=grad; ctx.lineWidth=2.5; ctx.setLineDash([9,5]); ctx.stroke(); ctx.setLineDash([])
      // Chevrons
      for (const t of [0.28,0.58]) {
        const ax=(1-t)*(1-t)*hx+2*(1-t)*t*cpx+t*t*bx, ay=(1-t)*(1-t)*hy+2*(1-t)*t*cpy+t*t*by
        const t2=Math.min(t+0.02,1), ax2=(1-t2)*(1-t2)*hx+2*(1-t2)*t2*cpx+t2*t2*bx, ay2=(1-t2)*(1-t2)*hy+2*(1-t2)*t2*cpy+t2*t2*by
        ctx.save(); ctx.translate(ax,ay); ctx.rotate(Math.atan2(ay2-ay,ax2-ax))
        ctx.strokeStyle='rgba(255,255,255,0.9)'; ctx.lineWidth=2
        ctx.beginPath(); ctx.moveTo(-7,-4); ctx.lineTo(0,0); ctx.lineTo(-7,4); ctx.stroke(); ctx.restore()
      }
      // Aim crosshair
      const aimX=hx+perpX*(an.breakIntensity*6*sign)*2, aimY=hy+perpY*(an.breakIntensity*6*sign)*2
      ctx.beginPath(); ctx.arc(aimX,aimY,13,0,Math.PI*2); ctx.strokeStyle='rgba(255,183,77,0.35)'; ctx.lineWidth=1; ctx.stroke()
      ctx.beginPath(); ctx.arc(aimX,aimY,6,0,Math.PI*2); ctx.fillStyle='rgba(255,183,77,0.9)'; ctx.fill(); ctx.strokeStyle='white'; ctx.lineWidth=1.5; ctx.stroke()
      ctx.strokeStyle='rgba(255,183,77,0.75)'; ctx.lineWidth=1.5
      for (const [x1,y1,x2,y2] of [[aimX,aimY-17,aimX,aimY-12],[aimX,aimY+12,aimX,aimY+17],[aimX-17,aimY,aimX-12,aimY],[aimX+12,aimY,aimX+17,aimY]] as [number,number,number,number][]) {
        ctx.beginPath(); ctx.moveTo(x1,y1); ctx.lineTo(x2,y2); ctx.stroke()
      }
      // Ball marker
      ctx.shadowColor='rgba(255,255,255,0.5)'; ctx.shadowBlur=8
      ctx.beginPath(); ctx.arc(bx,by,8,0,Math.PI*2); ctx.fillStyle='white'; ctx.fill(); ctx.shadowBlur=0; ctx.strokeStyle='#333'; ctx.lineWidth=1.5; ctx.stroke()
      // Hole ring
      ctx.beginPath(); ctx.arc(hx,hy,20,0,Math.PI*2); ctx.strokeStyle='rgba(76,175,80,0.5)'; ctx.lineWidth=1.5; ctx.setLineDash([3,3]); ctx.stroke(); ctx.setLineDash([])
      ctx.beginPath(); ctx.arc(hx,hy,8,0,Math.PI*2); ctx.fillStyle='#111'; ctx.fill(); ctx.strokeStyle='white'; ctx.lineWidth=1.5; ctx.stroke()
    }
  ...
  function onBallPointerUp(e: React.PointerEvent) {
    if (!ballDragRef.current.active) return
    e.stopPropagation(); ballDragRef.current.active = false
    if (!ballDragRef.current.moved) {
      haptic()
      setPhase('slope')   // ← go to slope measurement before walking
      ...
      } else { setGpsAvailable(false) }
    }
  }

  function handleSlopeMeasured(reading: SlopeReading) {
    haptic(); setSlope(reading); setPhase('walk')
  }
  function handleSlopeSkipped() {
    haptic(); setSlope(null); setPhase('walk')
  }

  function handleAtHole() { haptic(); setPhase('athole') }

  async function handleCapture() {
    if (!videoRef.current) return
    haptic(); setPhase('analyzing')
    ...
    try {
      const res = await fetch('/api/read-green', {
        ...
        body: JSON.stringify({
          image: imageBase64, ballPos, holePos,
          distanceFt, perspective: 'from_hole',
          slopeDegrees: slope?.slopeDegrees,
          crossSlopeDegrees: slope?.crossSlopeDegrees,
          confirmedGreenSpeed,
        }),
      })
      const data = await res.json()
      setAnalysis(data.analysis); setPhase('done')
      onAnalysisComplete({ ballPos, holePos, analysis: data.analysis, distanceFt, slopeDegrees: slope?.slopeDegrees, crossSlopeDegrees: slope?.crossSlopeDegrees })
    } catch {
      const fallback: GreenAnalysis = { breakDirection:'straight', breakIntensity:2, greenSpeed: confirmedGreenSpeed ?? 10, slope:'flat', grain:'neutral', confidence:30, notes:'Analysis failed — using defaults' }
      setAnalysis(fallback); setPhase('done')
      onAnalysisComplete({ ballPos, holePos, analysis: fallback, distanceFt, slopeDegrees: slope?.slopeDegrees, crossSlopeDegrees: slope?.crossSlopeDegrees })
    }
    haptic()
  }

  function handleReset() {
    haptic(); setBallPos({ x:0.5, y:0.72 }); setWalkedFt(null); setGpsAvailable(true)
    setSlope(null); setAnalysis(null); ballGPSRef.current = null; holeGPSRef.current = null
    setPhase('ball'); onReset()
  }

  const hideCamera = phase === 'slope' || phase === 'walk'

  return (
    <div className="flex flex-col gap-2">
      {/* Camera — always mounted so stream stays alive */}
      <div
        ref={containerRef}
        className={`relative w-full overflow-hidden rounded-2xl bg-green-950 select-none${hideCamera ? ' hidden' : ''}`}
        style={{ height: 340 }}
      >
        ...
        {cameraState === 'ready' && phase === 'ball' && (
          <div className="animate-pulse" style={{ position:'absolute', left:`${ballPos.x*100}%`, top:`${ballPos.y*100}%`, transform:'translate(-50%,-50%)', cursor:'grab', touchAction:'none', userSelect:'none' }}
            onPointerDown={onBallPointerDown} onPointerMove={onBallPointerMove} onPointerUp={onBallPointerUp}>
      ...
      {/* Slope measurement */}
      {phase === 'slope' && (
        <SlopeMeter onMeasured={handleSlopeMeasured} onSkip={handleSlopeSkipped} />
      )}
      ...
      {phase === 'walk' && (
        <div className="bg-green-900 rounded-2xl px-5 py-7 flex flex-col items-center gap-4">
          ...
          {slope && (
            <div className="bg-green-800 rounded-xl px-4 py-2 text-center">
              <p className="text-green-500 text-[9px] uppercase tracking-wider mb-0.5">Measured slope</p>
              <p className="text-green-200 text-sm font-bold font-mono">
                {Math.sqrt(slope.slopeDegrees**2 + slope.crossSlopeDegrees**2).toFixed(1)}° total
              </p>
            </div>
          )}
          {walkedFt !== null ? (
          ...
          <button onClick={handleAtHole} className="w-full py-3 rounded-xl bg-green-600 hover:bg-green-500 text-white font-bold text-sm transition-colors" style={{ minHeight:44 }}>
            I'm at the hole →
          </button>
        </div>
      )}
      ...
      {cameraState === 'ready' && (phase === 'athole' || phase === 'done') && (
        <div className="flex gap-2">
          {phase === 'athole' && (
            <button onClick={handleCapture} className="flex-1 py-3 rounded-xl bg-green-600 hover:bg-green-500 text-white font-bold text-sm transition-colors" style={{ minHeight:44 }}>
              🔍 Capture & analyze
            </button>
          )}
          {phase === 'done' && (
            <button onClick={handleReset} className="flex-1 py-3 px-4 rounded-xl border border-green-700 text-green-400 text-sm font-semibold hover:bg-green-900 transition-colors" style={{ minHeight:44 }}>
              ↺ Start over
            </button>
          )}
        </div>
